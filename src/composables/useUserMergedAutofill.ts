  ...
  const getUserInfos = async () => {
    ...
    try {
      const [autofillResult, dataDeletionResult] = (await Promise.allSettled([
        ...
        PromiseSettledResult<{ data: AutofillData }>,
        PromiseSettledResult<{ data: DataDeletionEnrollmentProfile }>,
      ];

      // Handle autofill result
      if (autofillResult.status === "fulfilled") {
        autofill.value = autofillResult.value.data;
      } else {
      ...
      if (dataDeletionResult.status === "fulfilled") {
        dataDeletionProfile.value = dataDeletionResult.value.data;
      } else {
        console.error(
