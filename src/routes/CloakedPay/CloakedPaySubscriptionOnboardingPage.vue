...
const isBackBtnDisabled = computed(() => {
  ...
  if (
    ...
    (kycData.kycFailureStatus?.state === "MAX_ATTEMPTS_REACHED" ||
      kycData.kycFailureStatus?.state === "PENDING" ||
...
const captureSectionScreenview = (step: StepSection) => {
  ...
  posthogCapture(`dashboard_post_checkout_pay_onboarding_${key}_screenview`);
};
...
function handleCloakedPayFormSave(payload: {
  ...
  posthogCapture(
    `dashboard_post_checkout_pay_onboarding_${eventKeyMap["cloaked-pay-application"]}_continue_clicked`
  );
...
const goBack = () => {
  ...
  posthogCapture(`dashboard_post_checkout_pay_onboarding_${key}_back_clicked`);
...
const openSupportPage = () => {
  posthogCapture("dashboard_post_checkout_pay_onboarding_support_fab_clicked");
...
const submitDataDeletionForm = (formData: DataDeletionFormData) => {
  ...
  posthogCapture(
    "dashboard_post_checkout_pay_onboarding_data_deletion_submit_clicked"
  );

  DataDeleteService.createEnrollmentData(toApiPayload(formData))
    .then(() => {
      posthogCapture(
        "dashboard_post_checkout_pay_onboarding_data_deletion_submit_success"
      );
    ...
    .catch((error) => {
      ...
      posthogCapture(
        "dashboard_post_checkout_pay_onboarding_data_deletion_submit_failed"
      );
    })
    .finally(() => {
...
const onCloakedPayKYCSubmit = () => {
  const key = eventKeyMap[currentSection.value];
  posthogCapture(
    `dashboard_post_checkout_pay_onboarding_${key}_submit_clicked`
  );
...
const submitCloakedPayApplication = () => {
  ...
  posthogCapture("dashboard_post_checkout_pay_onboarding_kyc_submitted");
  ...
  CardsServices.submitKYC(payload)
    .then(() => {
      ...
      setTimeout(() => {
        posthogCapture(
          "dashboard_post_checkout_pay_onboarding_kyc_submission_success"
        );
    ...
    .catch((error) => {
      setTimeout(() => {
        ...
        posthogCapture(
          "dashboard_post_checkout_pay_onboarding_kyc_submission_failed"
        );
      }, 3000);
    });
};
...
const handleWelcomeContinue = () => {
  posthogCapture(
    `dashboard_post_checkout_pay_onboarding_${eventKeyMap["welcome"]}_continue_clicked`
  );
...
const handleReviewContinue = () => {
  posthogCapture(
    `dashboard_post_checkout_pay_onboarding_${eventKeyMap["cloaked-pay-review"]}_continue_clicked`
  );
...
const handleReviewEditDetails = () => {
  posthogCapture(
    `dashboard_post_checkout_pay_onboarding_${eventKeyMap["cloaked-pay-review"]}_edit_details_clicked`
  );
  goToStep("cloaked-pay-application", "back");
};
...
const handleStatusPlanDowngraded = () => {
  posthogCapture(
    `dashboard_post_checkout_pay_onboarding_${eventKeyMap["cloaked-pay-status"]}_plan_downgrade_success`
  );
...
const handleStatusPlanDowngradedSkipped = () => {
  posthogCapture(
    `dashboard_post_checkout_pay_onboarding_${eventKeyMap["cloaked-pay-status"]}_plan_downgrade_skipped`
  );
  goToStep("data-deletion-application", "forward");
};
...
const handleStatusContinue = () => {
  posthogCapture(
    `dashboard_post_checkout_pay_onboarding_${eventKeyMap["cloaked-pay-status"]}_continue_clicked`
  );
  goToStep("data-deletion-application", "forward");
};
...
const handleStatusTryAgain = () => {
  posthogCapture(
    `dashboard_post_checkout_pay_onboarding_${eventKeyMap["cloaked-pay-status"]}_try_again_clicked`
  );
  goToStep("cloaked-pay-application", "back");
};
...
const handleOnboardingFinishProfile = () => {
  posthogCapture(
    `dashboard_post_checkout_pay_onboarding_${eventKeyMap["cloaked-pay-onboarding"]}_finish_profile_clicked`
  );
  goToStep("data-deletion-application", "forward");
};
...
const handleOnboardingSetupFirstCard = () => {
  posthogCapture(
    `dashboard_post_checkout_pay_onboarding_${eventKeyMap["cloaked-pay-onboarding"]}_setup_first_card_clicked`
  );
...
const handleSuccessSetupVirtualCard = () => {
  posthogCapture(
    `dashboard_post_checkout_pay_onboarding_${eventKeyMap["success"]}_setup_virtual_card_clicked`
  );
...
const handleSuccessContinue = () => {
  posthogCapture(
    `dashboard_post_checkout_pay_onboarding_${eventKeyMap["success"]}_continue_clicked`
  );
...
const handleExitModalOpened = () => {
  const key = eventKeyMap[currentSection.value];
  posthogCapture(
    `dashboard_post_checkout_pay_onboarding_${key}_exit_modal_opened`
  );
};
...
const handleExitModalConfirmed = () => {
  const key = eventKeyMap[currentSection.value];
  posthogCapture(
    `dashboard_post_checkout_pay_onboarding_${key}_exit_modal_confirmed`
  );
};
</script>
