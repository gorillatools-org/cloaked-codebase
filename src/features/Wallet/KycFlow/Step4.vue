...
function handleKYCButton(action: KYCStatusButtonAction | undefined) {
  ...
  if (action === "close") {
    posthogCapture(
      `dashboard_pay_wallet_kyc_flow_kyc_${pageStateName}_back_to_dashboard_button_clicked`
    );
    emit("backToDashboard");
  } else if (action === "emailSupport") {
    posthogCapture(
      `dashboard_pay_wallet_kyc_flow_kyc_${pageStateName}_contact_support_button_clicked`
    );
    openSupportLink();
  } else {
    posthogCapture(
      `dashboard_pay_wallet_kyc_flow_kyc_${pageStateName}_try_again_button_clicked`
    );
    emit("tryAgain");
  }
}
  ...
  CardsServices.postActiveKYC(payload)
    ...
    .catch((error) => {
      ...
      if (kycFailureStatus.value?.state) {
        posthogCapture(
          `dashboard_pay_wallet_kyc_flow_kyc_${kycFailureStatus.value?.state?.toLowerCase()}_viewed`
        );
      }
