...
function handleKYCButton(action: KYCStatusButtonAction | undefined) {
  ...
  if (action === "close") {
    posthogCapture(
      `dashboard_post_checkout_pay_onboarding_kyc_status_${pageStateName}_continue_to_next_step_button_clicked`
    );
    emit("continue");
  } else if (action === "emailSupport") {
    posthogCapture(
      `dashboard_post_checkout_pay_onboarding_kyc_status_${pageStateName}_contact_support_button_clicked`
    );
    openSupportLink();
  } else {
    posthogCapture(
      `dashboard_post_checkout_pay_onboarding_kyc_status_${pageStateName}_try_again_button_clicked`
    );
    emit("tryAgain");
  }
}
...
watch(
  ...
  (state: KYCState | undefined) => {
    posthogCapture(
      `dashboard_post_checkout_pay_onboarding_kyc_status_${(state || "loading").toLowerCase()}_screenview`
    );
  },
  { immediate: true }
);
