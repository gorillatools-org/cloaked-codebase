...
const { kycData } = storeToRefs(virtualCardsApplication);
...
function handleKYCButton(action: KYCStatusButtonAction | undefined) {
  ...
  if (action === "close") {
    posthogCapture(`dashboard_pay_kyc_${pageStateName}_close_button_clicked`);
    emit("close");
  } else if (action === "emailSupport") {
    posthogCapture(
      `dashboard_pay_kyc_${pageStateName}_contact_support_button_clicked`
    );
    openSupportLink();
  } else {
    posthogCapture(
      `dashboard_pay_kyc_${pageStateName}_try_again_button_clicked`
    );
    emit("tryAgain");
  }
}
...
watch(
  ...
  (state: KYCState | undefined) => {
    posthogCapture(
      `dashboard_pay_kyc_${(state || "loading").toLowerCase()}_screenview`
    );
  },
  { immediate: true }
);
