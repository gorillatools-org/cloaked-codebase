...
onMounted(() => {
  posthogCapture("dashboard_pay_kyc_confirm_plan_screenview");
});

const plans = computed(() => {
  return virtualCardsApplication.upgradePlanOptions;
});
...
const recommendedMonthlyPlanId = computed(() => {
  return getRecommendedPlan(monthlyPlans.value)?.product_id;
});
...
function onSwitchPlan() {
  posthogCapture("dashboard_pay_kyc_confirm_plan_switch_clicked");
  openSelectPlanModal();
}

function onConfirmPlan() {
  posthogCapture("dashboard_pay_kyc_confirm_plan_confirm_clicked");
  emit("confirm");
}
...
const openSelectPlanModal = () => {
  const modalProps = reactive({
    ...
    primaryMonthlyPlanId: recommendedMonthlyPlanId,
    ...
    onSelectPlan: (plan: Plan) => {
      ...
      posthogCapture("dashboard_pay_kyc_confirm_plan_select_different_plan");
  ...
  SubscriptionService.getPreviewSubscriptionChange(selectedPlan.product_id)
    .then((response: SubscriptionPreview) => {
      ...
      billingDetails.subtotal =
        selectedPlan.default_price || selectedPlan.price;
      billingDetails.dueToday = response.total;

      billingDetails.savings = [
        {
          amount: selectedPlan.price - (selectedPlan.default_price || 0),
          description: "Early access savings",
        },
      ];
