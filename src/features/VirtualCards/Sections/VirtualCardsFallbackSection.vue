...
onMounted(() => {
  posthogCapture(`dashboard_pay_fallback_page_viewed`);
});

async function refreshPage() {
  await posthogCapture(`dashboard_pay_fallback_page_refresh_button_clicked`);
  window.location.reload();
}

function handleSupportEmailCopied() {
  posthogCapture(`dashboard_pay_fallback_page_support_email_copied`);
}
</script>
