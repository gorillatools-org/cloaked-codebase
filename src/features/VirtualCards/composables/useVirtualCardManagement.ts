...
export default function useVirtualCardManagement(
  ...
  const lockCard = () => {
    return updateLockState("locked_by_user")
      .then(() => {
        posthogCapture("dashboard_pay_wallet_lock_card_success");
      })
      .catch(() => {
        posthogCapture("dashboard_pay_wallet_lock_card_failed");
      });
  };
  ...
  const unlockCard = () => {
    return updateLockState("unlocked")
      .then(() => {
        posthogCapture("dashboard_pay_wallet_unlock_card_success");
      })
      .catch(() => {
        posthogCapture("dashboard_pay_wallet_unlock_card_failed");
      });
  };
  ...
  const lockMerchant = () => {
    return updateMerchantLockState(true)
      .then(() => {
        posthogCapture("dashboard_pay_wallet_lock_merchant_success");
      })
      .catch(() => {
        posthogCapture("dashboard_pay_wallet_lock_merchant_failed");
      });
  };
  ...
  const unlockMerchant = () => {
    return updateMerchantLockState(false)
      .then(() => {
        posthogCapture("dashboard_pay_wallet_unlock_merchant_success");
      })
      .catch(() => {
        posthogCapture("dashboard_pay_wallet_unlock_merchant_failed");
      });
  };
  ...
  const updateCardLimit = (
    ...
    return _updateCardLimit(limitCents, period, fixedUses)
      .then(() => {
        ...
        posthogCapture("dashboard_pay_wallet_update_card_limit_success", {
          limit: limitCents,
          period,
        });
      })
      .catch((error) => {
        posthogCapture("dashboard_pay_wallet_update_card_limit_failed");
        throw error;
      });
  };
  ...
  const updateSelfDestructDate = (expiresAtType: VirtualCardExpiresAt) => {
    ...
    return _updateSelfDestructDate(newDate.toDate())
      .then(() => {
        refetchCards(true);
        posthogCapture(
          "dashboard_pay_wallet_update_self_destruct_date_success",
          {
      ...
      .catch(() => {
        posthogCapture(
          "dashboard_pay_wallet_update_self_destruct_date_failed",
          {
            period: expiresAtType,
          }
        );
      ...
      customTemplate: {
        ...
        props: {
          ...
          onConfirm: () => {
            ...
            cancelCard()
              .then(() => {
                posthogCapture("dashboard_pay_wallet_cancel_card_success");
                refreshCreationLimit();
                refetchCards(false);
                if (onDeleted) {
                  onDeleted();
              ...
              .catch(() => {
                posthogCapture("dashboard_pay_wallet_cancel_card_failed");
              })
              .finally(() => {
