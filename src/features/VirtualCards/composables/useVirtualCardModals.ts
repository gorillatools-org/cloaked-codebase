...
import SpendingLimitModal from "@/features/Wallet/WalletSettings/SpendingLimitModal.vue";
...
import { PH_VIRTUAL_CARDS_FEATURE_FLAG_EXPRESS_CARD_GENERATION } from "../constants/posthog-feature-flag";
...
export default function useVirtualCardModals(
  ...
  const openSpendingLimitModal = () => {
    ...
    if (isExpressCardGenerationEnabled.value) {
      store.dispatch("openModal", {
        customTemplate: {
    ...
    store.dispatch("openModal", {
      customTemplate: {
        template: markRaw(SpendingLimitModal),
        props: {
          show: true,
          onClose: () => {
