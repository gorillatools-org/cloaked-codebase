...
const isFormValid = computed(() => {
  if (type.value !== "phone") {
    return (
      ...
      (username.value.length > 0 || email.value.length > 0)
    );
  }

  return !primaryIdentifierError.value && phone.value.length > 0;
});

const toInternationalPhoneNumber = (phoneNumber) => {
...
<template>
  <div class="choose-plan-signup">
    ...
    <CheckoutParagraph v-if="type === 'phone' && !isRealUser">
      Use your phone number to create an account. It will also be your username
      to login.
    </CheckoutParagraph>
